...
export default class TodoList extends Component {
  ...
  removeTodo(todoId) {
    ...
    const mainTodoIndex = newTodos.findIndex(todo => todo.id === todoId);
    newTodos.splice(mainTodoIndex, 1);
          ...
          <ul className='todo-list'>
            {this.state.status === 'completed' &&
              this.state.todos
                ...
                .map(todo => <Todo key={todo.id} {...todo} onRemove={this.removeTodo} onEdit={this.editTodo} />)}
            ...
            {this.state.status === 'all' &&
              this.state.todos.map(todo => <Todo key={todo.id} {...todo} onRemove={this.removeTodo} onEdit={this.editTodo} />)}
          </ul>
        </div>
      </>
    );
